...
class WhoisResult(ParsedResult):
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.domain_info:
            ...
            result['domain_info']['registrant_contact'] = self.domain_info.registrant_contact.__dict__
            result['domain_info']['admin_contact'] = self.domain_info.admin_contact.__dict__
            result['domain_info']['tech_contact'] = self.domain_info.tech_contact.__dict__
...
class WhoisParser(ToolResultsParser):
    ...
    def parse(self, target: str) -> WhoisResult:
        ...
        if is_rpsl:
            ...
            try:
                result.diagnostics = self._compute_diagnostics({"registrar": getattr(domain_info, "registrar", "")}, {"raw_output": raw_output})
            except Exception:
                result.diagnostics = []
            return result
        ...
        if is_ip:
            ...
            try:
                result.diagnostics = self._compute_diagnostics(None, {"raw_output": raw_output})
            except Exception:
                result.diagnostics = []
            return result
        else:
        ...
        for line in processed_lines:
            ...
            if ':' in line:
                ...
                if 'domain_name' in key or key == 'domain':
                    ...
                    clean_domain = value.split()[0].lower() if value.split() else value.lower()
                ...
                # Dates (prefer the more recent/detailed format)
                    ...
                    if normalized_date and (not domain_info.creation_date or len(normalized_date) > len(domain_info.creation_date)):
                        domain_info.creation_date = normalized_date
                elif any(date_key in key for date_key in ['updated_date', 'updated', 'last_modified', 'changed', 'last_updated']):
                    normalized_date = self._normalize_date(value)
                    if normalized_date and (not domain_info.updated_date or len(normalized_date) > len(domain_info.updated_date)):
                        domain_info.updated_date = normalized_date
                elif any(date_key in key for date_key in ['expiry_date', 'expires', 'expiration', 'registry_expiry_date']):
                    normalized_date = self._normalize_date(value)
    ...
    def _compute_diagnostics(self,
        ...
        if not ruleset:
            try:
                ...
                env_path = os.environ.get("WHOIS_RULESET_PATH") or os.environ.get("WHOIS_RULES")
                if env_path:
        ...
        raw = (scan_stats.get("raw_output") or getattr(self, "get_raw_output", lambda: "")() or "").strip()

        # try extract Registrar lines from raw
        for m in re.finditer(r'^\s*Registrar:\s*(.+)$', raw, re.IGNORECASE | re.MULTILINE):
            registrar_candidates.append(m.group(1).strip())
        for m in re.finditer(r'^\s*Registrar Name:\s*(.+)$', raw, re.IGNORECASE | re.MULTILINE):
            registrar_candidates.append(m.group(1).strip())

        # normalize registrar list
        registrar_candidates = [r.rstrip('.').strip() for r in registrar_candidates if r and str(r).strip()]
        seen = set()
        regs = []
        ...
        registrar_rules = ruleset.get("registrar_rules") if isinstance(ruleset, dict) else []
        for reg in regs:
            for rule in (registrar_rules or []):
                ...
                try:
                    if matches(pattern, reg):
                        ...
                        msg = rule.get("message") or f"Registrar rule matched: {pattern}"
        ...
        m = re.search(r'Creation Date:\s*([0-9]{4})-([0-9]{2})-([0-9]{2})', raw, re.IGNORECASE)
        if not m:
            m = re.search(r'Created On:\s*([0-9]{4})-([0-9]{2})-([0-9]{2})', raw, re.IGNORECASE)
        if m:
            try:
        ...
        m2 = re.search(r'(Updated Date|Updated):\s*([0-9]{4})-([0-9]{2})-([0-9]{2})', raw, re.IGNORECASE)
        if m2:
            try:
                updated_year = int(m2.group(2))
                updated_full = f"{m2.group(2)}-{m2.group(3)}-{m2.group(4)}" if False else f"{m2.group(2)}-{m2.group(3)}-{m2.group(4)}"
            except Exception:
                updated_year = None

        # alternative Updated patterns (some WHOIS outputs use "Updated Date: YYYY-MM-DD")
        if updated_year is None:
            m3 = re.search(r'Updated Date:\s*([0-9]{4})-([0-9]{2})-([0-9]{2})', raw, re.IGNORECASE)
            if m3:
                try:
        ...
        for rule in (age_rules or []):
            ...
            try:
                ...
                if year_val is None and re.search(r'creation', pattern, re.IGNORECASE) and creation_year:
                    year_val = creation_year
                if year_val is None and re.search(r'update|updated', pattern, re.IGNORECASE) and updated_year:
            ...
            try:
                if condition:
                    ...
                    expr = condition.replace("\\1", str(year_val)).replace("\\\\1", str(year_val))
                    # expose age, current_year, year for expression
                    local_vars = {"age": age, "current_year": current_year, "year": year_val}
            ...
            if matched:
                ...
                ctx = f"creation_year={creation_year}" if creation_year else f"year={year_val}"
                add(sev, msg, ctx)
