...
class WhatWebParser:
    ...
    def parse(self, target: str) -> WhatWebResult:
        ...
        try:
            ...
            parsed_data["diagnostics"] = self._compute_diagnostics(
    ...
    def _parse_standard(self, lines: List[str], target: str, raw_output: str) -> Dict:
        ...
        for line in lines:
            ...
            if (
                ...
                or line.startswith("Started scan")
                or line.startswith("[SCAN COMPLETED]")
            ):
                continue
            ...
            if url_match:
                ...
                for match in re.finditer(tech_pattern, rest):
                    ...
                    if len(name) > 1 and not name.startswith("http"):
                        technologies.append({"name": name, "info": info})
        ...
        for report in parsed.get("reports", []):
            ...
            if report.get("plugins"):
                ...
                for plugin in report["plugins"]:
                    ...
                    if plugin.get("description"):
                        desc = plugin["description"][:200] + (
                            "..." if len(plugin["description"]) > 200 else ""
                        )
            ...
            if report.get("http_headers"):
                ...
                for header in report["http_headers"][:20]:  # Limit to first 20
                    html += (
                        f"<div style='color: #b0bec5; padding: 2px 0;'>{header}</div>"
                    )
    ...
    def _compute_diagnostics(
        ...
        if not ruleset:
            try:
                ...
                candidates = [
                    ...
                    os.path.join(
                        os.getcwd(), "src", "rulesets", "whatweb_ruleset.json"
                    ),
                    os.path.join(os.getcwd(), "src", "whatweb_ruleset.json"),
                ]
                env_path = os.environ.get("WHATWEB_RULESET_PATH") or os.environ.get(
                    "WEB_RULESET_PATH"
                )
                if env_path:
        ...
        for f in findings or []:
            if not isinstance(f, dict):
                continue
            if f.get("url") and (
                "plugins" in f or "http_headers" in f or "technologies" in f
            ):
                reports.append(f)
            elif f.get("url"):
        ...
        raw_text = (
            scan_stats.get("raw_output") or raw_output or self.get_raw_output() or ""
        ).strip()
        ...
        if reports:
            ...
            for p in first.get("plugins") or []:
                if isinstance(p, dict):
                    ...
                    for s in p.get("strings") or []:
                        if isinstance(s, str):
                            if (
                                s.lower().startswith("set-cookie")
                                or "cookie" in s.lower()
                            ):
            ...
            server_header = (
                headers_map.get("server", "")
                or first.get("server", "")
                or first.get("title", "")
                or ""
            )
            if (
                first.get("status_code")
                and 300 <= int(first.get("status_code") or 0) < 400
            ):
        ...
        if raw_text:
            tls_list.extend(
                re.findall(
                    r"(tls|ssl|https|x509|certificate|cipher|openssl)[^ \n,;]{0,30}",
                    raw_text,
                    re.IGNORECASE,
                )
            )
            found_files.extend(
                re.findall(
                    r"(?:/[\w\-/]+(?:robots\.txt|sitemap\.xml|backup|\.env|\.git|README\.md|composer\.json))",
                    raw_text,
                    re.IGNORECASE,
                )
            )
            cookies_list.extend(
                re.findall(r"Set-Cookie:\s*([^;\n\r]+)", raw_text, re.IGNORECASE)
            )
            redirects.extend(
                re.findall(r"Location:\s*([^\s]+)", raw_text, re.IGNORECASE)
            )
        ...
        group_sources = {
            ...
            "security_header_rule": lambda rule: " ".join(
            ...
            "uncommon_header_rule": lambda rule: " ".join(
                [f"{k}:{v}" for k, v in hdrs.items()]
            )
            + " "
            + raw_text_lower,
            ...
            "cms_plugin_rule": lambda rule: " ".join(plugins_list)
            + " "
            + raw_text_lower,
            "exposed_file_rule": lambda rule: " ".join(found_files)
            + " "
            + raw_text_lower,
                ...
                "server_rule": [
                    {
                        "pattern": "varnish",
                        "severity": "Info",
                        "message": "Server header indicates Varnish (fallback test rule)",
                    }
                ]
            }
            self.whatweb_ruleset = ruleset
        ...
        try:
            server_rules = (
                ruleset.get("server_rule", []) if isinstance(ruleset, dict) else []
            )
            if server_rules:
            ...
            for rule in rules:
                try:
                    ...
                    if not pattern:
                        continue
                    severity = (
                        rule.get("severity") or defaults.get("severity") or "Info"
                    )
                    message = (
                        rule.get("message")
                        or defaults.get("message")
        ...
        if (
            "directory_listing_rule" not in ruleset
            or not ruleset.get("directory_listing_rule")
        ) and re.search(r"Index of /|<title>\s*Index of", raw_text, re.IGNORECASE):
