...
class NmapResultsParser(ToolResultsParser):
    def __init__(self):
        ...
        try:
            ...
            candidates.append(os.path.join(here, "..", "nmap_ruleset.json"))
            candidates.append(os.path.join(here, "..", "rulesets", "nmap_ruleset.json"))
            candidates.append(os.path.join(os.getcwd(), "src", "rulesets", "nmap_ruleset.json"))
            candidates.append(os.path.join(os.getcwd(), "rulesets", "nmap_ruleset.json"))
        ...
        try:
            svc_map = (self.ruleset or {}).get("service_map", {}) if isinstance(self.ruleset, dict) else {}
            for k, v in (svc_map or {}).items():
                try:
                    ...
                    try:
                        self.compiled_service_map.append((re.compile(re.escape(k), re.IGNORECASE), v))
                    except Exception:
                        continue
        except Exception:
            self.compiled_service_map = []
    ...
    def parse(self, target: str) -> NmapResult:
        ...
        diagnostics = self._compute_diagnostics(hosts, ports, os_info, ssl_info, scan_stats)
    ...
    def _parse_hosts_and_ports(self, hosts: List[HostInfo], ports: List[PortInfo]):
        ...
        for idx, raw_line in enumerate(self.raw_lines):
            ...
            m_other = re.match(r'Other addresses for\s+([^\s]+)(?:\s*\([^)]+\))?:\s*(.*)', line, re.IGNORECASE)
            if m_other:
            ...
            if port_match:
                ...
                service_token = port_match.group(4)   # original SERVICE token (e.g. "http", "ssl/https")
                extra = (port_match.group(5) or "").strip()  # may contain product/version info (sV)
                ...
                if scripts:
                    ...
                    def _ingest_request_block(req_name: str, lines: List[str]):
                        ...
                        norm_lines = [re.sub(r'^\s*\|\s*', '', (ln or "")).rstrip() for ln in lines]
                        ...
                        while i < len(cleaned):
                            ...
                            m_status = re.match(r'^HTTP/\d+\.\d+\s+(\d{3})\s*(.*)$', s, re.IGNORECASE)
                            ...
                            if m_header:
                                ...
                                if lh == "server" and not server:
                                    server = v
                                    continue
                                if lh in ("x-served-by", "x-served-by:") and not x_served_by:
                                    x_served_by = v
                                    continue
                        ...
                        if not status_code:
                            for idx_n, nl in enumerate(list(notes_lines)):
                                m2 = re.search(r'HTTP/\d+\.\d+\s+(\d{3})\s*(.*)$', nl, re.IGNORECASE)
                                if m2:
                        ...
                        combined = "\n".join(notes_lines)
                        title_m = re.search(r'<title[^>]*>(.*?)</title>', combined, re.IGNORECASE | re.DOTALL)
                        if title_m:
                        ...
                        fastly_m = re.search(r'(Fastly error:[^\n\r]+)', combined, re.IGNORECASE)
                        if fastly_m:
                    ...
                    fs_raw = scripts.get("fingerprint-strings") or scripts.get("fingerprint-strings:") or ""
                    if fs_raw:
                        # Split into lines, normalize leading pipe chars
                        raw_lines = [re.sub(r'^\s*\|\s*', '', (ln or "")).rstrip() for ln in fs_raw.splitlines()]
                        ...
                        for line_fs in cleaned:
                            ...
                            if m_hdr:
                                ...
                                cur_names = [n.strip() for n in re.split(r',\s*', name_field) if n.strip()]
                                cur_block = []
                                continue
                    ...
                    if not request_rows:
                        ...
                        title_val = scripts.get("http-title") or scripts.get("http-title:") or ""
                        server_val = scripts.get("http-server-header") or scripts.get("http-server-header:") or ""
                        if title_val:
                        ...
                        ssl_cert_val = scripts.get("ssl-cert") or scripts.get("ssl-cert:")
                        if ssl_cert_val:
                            # include CN and notable SAN/cert lines
                            cn_m = re.search(r'commonName=([^/,\n\r]+)', ssl_cert_val, re.IGNORECASE)
                            if cn_m:
                                generic_notes.append(f"Cert CN: {cn_m.group(1).strip()}")
                        if generic_notes:
                            request_rows.append({
                                "request_type": "probe",
                                "status_code": "",
                    ...
                    http_server = scripts.get("http-server-header") or scripts.get("http-server-header:")
                    if http_server:
            ...
            if not rdns_match:
                rdns_match = re.search(r'reverse\s*dns\s*(?:record)?\s*for\s+([\d\.]+):\s*(\S+)', line, re.IGNORECASE)
            if rdns_match:
    ...
    def _parse_os_detection(self, os_info: List[OSInfo]):
        ...
        def _parse_guess_list(text: str) -> List[OSInfo]:
            ...
            for part in parts:
                # Try: "Name Version (NN%)"
                m_full = re.match(r'^(?P<name>.+?)\s+(?P<version>[0-9A-Za-z\.\-X]+)\s*\((?P<acc>\d+)%\)\s*$', part)
                if m_full:
                ...
                if m_acc:
                    ...
                    if m_nv:
                        items.append(OSInfo(name=m_nv.group('n').strip(), version=m_nv.group('v').strip(), accuracy=acc))
                    else:
                        items.append(OSInfo(name=name, accuracy=acc))
                    continue

                # Try: "Name Version" (no accuracy)
                m_nv2 = re.match(r'^(?P<name>.+?)\s+(?P<version>[0-9A-Za-z\.\-X]+)\s*$', part)
                if m_nv2:
                    items.append(OSInfo(name=m_nv2.group('name').strip(), version=m_nv2.group('version').strip(), accuracy=0))
                    continue
        ...
        for raw_line in self.raw_lines:
            ...
            m_running_guess = re.match(r'^\s*Running\s*\(.*JUST\s*GUESSING.*\)\s*:\s*(.+)$', line, re.IGNORECASE)
            if m_running_guess:
            ...
            # Aggressive guesses
            m_aggr = re.match(r'^\s*Aggressive OS guesses\s*:\s*(.+)$', line, re.IGNORECASE)
            if m_aggr:
            ...
            if m_cpe:
                ...
                for t in tokens:
                    ...
                    cpe_entries.append({"raw": t, "vendor": vendor, "product": product, "version": ver})
                continue

            # Parse lines which look like 'Name (NN% accuracy)' appearing anywhere
            m_acc_any = re.match(r'^\s*(.+?)\s*\((\d+)%\s*accuracy\)\s*$', line, re.IGNORECASE)
            if m_acc_any:
                ...
                m_nv = re.match(r'^(?P<n>.*\D)\s+(?P<v>[0-9A-Za-z\.\-X]+)$', name)
                if m_nv:
                    os_info.append(OSInfo(name=m_nv.group('n').strip(), version=m_nv.group('v').strip(), accuracy=acc))
                else:
                    os_info.append(OSInfo(name=name, accuracy=acc))
                continue

            # If we hit a line that indicates OS detection finished, skip further parsing
            if line.lower().startswith("no exact os matches") or line.lower().startswith("os cpe:") or line.lower().startswith("nmap done:"):
        ...
        if cpe_entries:
            ...
            for os_entry in os_info:
                ...
                for c in cpe_entries:
                    ...
                    if (prod_l and prod_l in name_l) or (vend_l and vend_l in name_l) or (cver and cver in (os_entry.version or "")):
            ...
            for c in cpe_entries:
                already = any((getattr(o, "cpe", "") or "") == c.get("raw", "") for o in os_info)
                if not already:
    ...
    def _parse_scan_statistics(self, scan_stats: Dict[str, Any]):
        ...
        for line in self.raw_lines:
            ...
            m_not_shown = re.match(
                r'^\s*Not shown:\s*(\d+)\s+(.+?)\s+ports(?:\s*\(([^)]+)\))?\s*$', line, re.IGNORECASE
            )
            if m_not_shown:
                # count
                try:
    ...
    def _parse_ssl_info(self, ssl_info: List[SSLInfo]):
        ...
        for idx, line in enumerate(lines):
            l = line.strip()
            if not any(token in l.lower() for token in ("ssl", "tls", "certificate", "subject", "issuer", "cipher")):
                continue
    ...
    def _is_scan_successful(self, *args, **kwargs):
        ...
        if len(args) == 2 and isinstance(args[1], dict):
            ...
            if isinstance(stats, dict):
                ...
                if any(tok in status for tok in ("error", "failed", "timeout", "refused")):
                    return False
            return bool(parsed)
        ...
        if isinstance(stats, dict):
            ...
            if any(tok in status_text for tok in ("error", "failed", "timed out", "timeout", "refused")):
                return False
    ...
    def _parse_traceroute(self) -> List[Dict[str, Any]]:
        ...
        for idx, raw in enumerate(lines):
            ...
            if not in_tr:
                ...
                if re.search(r'^\s*TRACEROUTE\b', line, re.IGNORECASE) or re.search(r'\btraceroute\b', line, re.IGNORECASE) and 'hop' in line.lower():
                    in_tr = True
                    continue
            ...
            if not m:
                ...
                if line.strip() in ("...", "*", "* * *"):
                    ...
                    hops.append({"Hop": "", "RTT (ms)": "", "Address": "", "Hostname": "", "Notes": "no response"})
                continue
            ...
            if rest.startswith('...') or rest.startswith('*') or rest == '':
                hops.append({"Hop": hop_num, "RTT (ms)": "", "Address": "", "Hostname": "", "Notes": "no response"})
                continue
            ...
            if re.search(r'\b(no reply|no response)\b', rest, re.IGNORECASE) or '*' in rest:
                if not notes_list:
    ...
    def _compute_diagnostics(self, hosts: List[HostInfo], ports: List[PortInfo], os_info: List[OSInfo], ssl_info: List[SSLInfo], scan_stats: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Produce diagnostics by comparing parsed results to the loaded ruleset."""
        diags: List[Dict[str, Any]] = []
        try:
            rules = (self.ruleset or {}).get("rules", []) if isinstance(self.ruleset, dict) else []
        except Exception:
            rules = []
        defaults = (self.ruleset or {}).get("defaults", {}) if isinstance(self.ruleset, dict) else {}
        version_rules = (self.ruleset or {}).get("version_rules", []) if isinstance(self.ruleset, dict) else []
        ...
        for p in ports or []:
            ...
            for r in rules:
                ...
                contains_match = bool(svc_contains and (svc_contains in p_svc or svc_contains in p_extra))

                if proto_ok and (port_match or service_match or contains_match):
                    add(r.get("severity"), r.get("message", "Matched rule"), f"port {p.port}/{p.protocol} service={p.service}")
                    matched = True

            # simple fallback heuristics if nothing matched
            if not matched:
                heuristics = {
                    "ftp": ("High", "FTP transmits credentials in clear text; secure with TLS or disable."),
                    "telnet": ("High", "Telnet is unencrypted; avoid."),
                    "mysql": ("High", "Public MySQL access can expose data."),
                    "redis": ("High", "Unprotected Redis can allow remote writes/execution."),
                ...
                for tok, info in heuristics.items():
                    if tok in p_svc or tok in p_extra:
                        add(info[0], info[1], f"port {p.port}/{p.protocol} service={p.service}")
        ...
        for vr in version_rules or []:
            ...
            if not vr_min and not vr.get("pattern"):
                continue
            vr_service = (vr.get("service") or "").lower() if vr.get("service") else None
            vr_contains = (vr.get("service_contains") or "").lower() if vr.get("service_contains") else None
            ...
            for p in ports or []:
                ...
                if p.scripts:
                    try:
                        scripts_text = " ".join([f"{k} {v}" for k, v in p.scripts.items()])
                    except Exception:
                        scripts_text = " ".join([str(k) for k in (p.scripts.keys() if hasattr(p.scripts, "keys") else [])])
                combined = " ".join(filter(None, [(p.service or ""), (p.version or ""), (p.extra_info or ""), scripts_text])).lower()
                ...
                if vr_service:
                    msv = re.search(r'%s[\s/:-]*([0-9A-Za-z\.\-p_]+)' % re.escape(vr_service), combined, re.IGNORECASE)
                    if msv:
                        actual_ver = msv.group(1)
                # 2) try explicit version field or extra_info
                if not actual_ver:
                    actual_ver = _extract_version_token((p.version or "") or (p.extra_info or "") or combined)
                ...
                if not actual_ver and not vr_pattern:
                    ...
                    add("Info", f"version not detected for {vr.get('service') or vr.get('service_contains') or 'rule'}", f"port {p.port}/{p.protocol}")
                    continue
                ...
                # version comparison
                if actual_ver_cmp and vr_min and _cmp_versions(actual_ver_cmp, vr_min) < 0:
                    sev = vr.get("severity") or "High"
                    msg = vr.get("message") or f"{p.service or 'service'} version {actual_ver} is older than required {vr_min}"
                    ...
                    sev = vr.get("severity") or "High"
                    msg = vr.get("message") or f"{p.service or 'service'} matches pattern {vr_pattern}"
                    add(sev, msg, f"port {p.port}/{p.protocol} (detected {actual_ver or 'unknown'})")
        ...
        for si in ssl_info or []:
            ...
            if si.valid_to and "expired" in si.valid_to.lower():
                add("Medium", f"Certificate validity ends: {si.valid_to}", f"port {si.port}")
        ...
        for r in rules:
            ...
            for o in os_info or []:
                if r_os in (o.name or "").lower() or r_os in (o.cpe or "").lower():
                    add(r.get("severity"), r.get("message", "OS rule matched"), f"os: {o.name}")

        # scan_stats hints
        if scan_stats.get("not_shown_display"):
            add("Info", f"Not shown ports: {scan_stats.get('not_shown_display')}", "scan_stats")
