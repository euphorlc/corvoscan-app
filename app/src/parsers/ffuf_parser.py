...
class FFUFParser(ToolResultsParser):
    ...
    def parse(self, target: str) -> FFUFResult:
        ...
        def _add_entry(path: Optional[str], status: int = 0, size: int = 0, words: int = 0, lines_cnt: int = 0, dur_ms: Optional[int] = None) -> bool:
            ...
            entries_found.append(DirectoryEntry(path=canon, status_code=status, size=size, words=words, lines=lines_cnt, duration_ms=dur_ms))
            return True
        ...
        try:
            ...
            if objs:
                if len(objs) == 1:
                    ...
                    comb = {'results': []}
                    for o in objs:
                        if isinstance(o, dict) and 'results' in o and isinstance(o['results'], list):
            ...
            try:
                ...
                for r in parsed.get('results', []):
                    ...
                    if isinstance(r.get('input'), dict):
                        path = r.get('input', {}).get('url') or r.get('input', {}).get('path') or r.get('input', {}).get('value') or ""
                    else:
                        path = r.get('input') or r.get('uri') or r.get('url') or r.get('path') or ""
                    try:
                    ...
                    _add_entry(path, status=status, size=size, words=words, lines_cnt=lines_cnt, dur_ms=dur_ms)
                ...
                if isinstance(parsed.get('config'), dict):
                    ...
                    for k in ('method', 'url', 'follow', 'timeout', 'threads', 'matcher', 'calibration'):
                        if k in cfg and cfg.get(k) not in (None, ""):
                            metadata[k if k != 'follow' else 'follow_redirects'] = str(cfg.get(k))
            except Exception:
        ...
        if isinstance(parsed, dict):
            ...
            if isinstance(p, dict):
                try:
                    progress_processed = int(p.get('processed') or p.get('current') or p.get('count') or progress_processed)
                except Exception:
                    pass
                try:
                    progress_total = int(p.get('total') or p.get('max') or p.get('total_requests') or progress_total)
                except Exception:
                    pass
                try:
                    requests_per_sec = float(str(p.get('rps') or p.get('requests_per_sec') or p.get('req_per_sec') or requests_per_sec).replace(',', ''))
                except Exception:
                    pass
                if p.get('duration') is not None:
                    progress_duration = str(p.get('duration'))
                try:
                    errors_count = int(p.get('errors') or p.get('errs') or errors_count or 0)
                except Exception:
                    pass
        ...
        re_tokens = re.compile(r'Status:\s*(?P<status>\d+).*?Size:\s*(?P<size>\d+)', re.IGNORECASE)
        re_progress = re.compile(r'Progress:\s*\[\s*(?P<processed>\d+)\s*/\s*(?P<total>\d+)\s*\].*?(?P<rps>[\d,]+(?:\.\d+)?)\s*req/sec.*?Duration:\s*\[(?P<dur>[^\]]+)\].*?Errors:\s*(?P<errs>\d+)', re.IGNORECASE)
        ...
        for raw_line in (self.raw_lines or raw_output.splitlines()):
            ...
            try:
                ...
                if m_simple:
                    ...
                    words = int(m_simple.group('words') or 0) if m_simple.group('words') else 0
                    lines_cnt = int(m_simple.group('lines') or 0) if m_simple.group('lines') else 0
                    dur_ms = int(m_simple.group('dur')) if m_simple.group('dur') else None
                    _add_entry(path, status=status, size=size, words=words, lines_cnt=lines_cnt, dur_ms=dur_ms)
            ...
            if (not in_results_table) and ('|' in line and 'Path' in line and 'Status' in line):
                in_results_table = True
                continue
            ...
            if not in_results_table:
                ...
                if m_header:
                    ...
                    if 'debug' in k_norm or k_norm == 'progress' or k_norm.startswith('progress '):
                        continue
                    ...
                    if 'method' in k_norm:
                    ...
                    elif 'timeout' in k_norm or 'maxtime' in k_norm or 'max-time' in k_norm:
                        canonical = 'timeout'
                        ...
                        if any(tok in k_norm for tok in ('filter', 'requests', 'duration', 'errors')):
                    ...
                    if canonical or not (_looks_like_ascii_art(key) or _looks_like_ascii_art(val)):
                ...
                if '|' in line:
                    ...
                    try:
                        if len(cols) >= 2:
                            ...
                            size = int(re.sub(r'[^\d]', '', cols[2])) if len(cols) > 2 and re.search(r'\d', cols[2]) else 0
                            words = int(re.sub(r'[^\d]', '', cols[3])) if len(cols) > 3 and re.search(r'\d', cols[3]) else 0
                            lines_cnt = int(re.sub(r'[^\d]', '', cols[4])) if len(cols) > 4 and re.search(r'\d', cols[4]) else 0
                            dur_ms = None
                ...
                if m_old:
                    try:
                        ...
                        if path and path not in seen_paths:
                            ...
                            words = int(m_old.group('words')) if m_old.group('words') else 0
                            lines_cnt = int(m_old.group('lines')) if m_old.group('lines') else 0
                            duration_ms = int(m_old.group('dur')) if m_old.group('dur') else None
                            entries_found.append(DirectoryEntry(path=path, status_code=status, size=size, words=words, lines=lines_cnt, duration_ms=duration_ms))
                        continue
                    except Exception:
                        pass
                ...
                if m_pb:
                    try:
                        ...
                        if path and path not in seen_paths:
                            ...
                            words = int(m_pb.group('words')) if m_pb.group('words') else 0
                            lines_cnt = int(m_pb.group('lines')) if m_pb.group('lines') else 0
                            duration_ms = int(m_pb.group('dur')) if m_pb.group('dur') else None
                            entries_found.append(DirectoryEntry(path=path, status_code=status, size=size, words=words, lines=lines_cnt, duration_ms=duration_ms))
                        continue
                    except Exception:
                        pass
            ...
            if 'Progress:' in line:
                ...
                progress_info["metadata"].setdefault("last_progress_line", line.strip()[:200])
                continue
            ...
            if not filter_criteria and re.search(r'\bfilter\b', line, re.IGNORECASE):
                ...
                if _looks_like_ascii_art_line(line):
                    ...
                    m = re.search(r'(?:\:\:\s*)?Filter\b\s*[:\-]?\s*(?P<val>.+)$', line, re.IGNORECASE)
                    if not m:
                        # fallback: sometimes shown as "Response filter: ..." or "Filter <something>"
                        m = re.search(r'Response\s+filter\b\s*[:\-]?\s*(?P<val>.+)$', line, re.IGNORECASE)
                    if m:
        ...
        if progress_info.get("metadata", {}).get("wordlist_raw"):
            progress_info.setdefault("wordlist", progress_info["metadata"].get("wordlist_raw"))
        ...
        if metadata:
            ...
            for k, v in metadata.items():
                ...
                elif k_lower in ('follow_redirects', 'follow redirects', 'follow-redirect', 'follow'):
        ...
        def _has_errors_simple() -> bool:
            ...
            if re.search(r'^\s*(error|fatal|failed|ffuf:)', raw_output, re.IGNORECASE | re.MULTILINE):
                return True
            if re.search(r'connection refused|no such file|no such host|permission denied', raw_output, re.IGNORECASE):
                return True
            return False
        ...
        try:
            result.diagnostics = self._compute_diagnostics(entries_found, {"raw_output": raw_output})
        except Exception:
            result.diagnostics = []
        return result
    ...
    def _extract_error_message(self) -> Optional[str]:
        for line in (self.raw_lines or []):
            ...
            if m and int(m.group(1)) > 0:
                return s
            if re.search(r'connection refused|no such file|no such host|permission denied', s, re.IGNORECASE):
                return s
        return None
                ...
                for rule in (path_rules or []):
                    try:
                        ...
                        if matches(rule_path, hay):
                            ...
                            msg = rule.get("message") or f"Path rule matched: {rule_path}"
