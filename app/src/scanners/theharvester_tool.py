...
try:
    ...
    extra_in_map = [
        ...
        if (k not in UI_PARAMS_EXPECTED and k != "Domain (-d, --domain) [REQUIRED]")
    ]
    if missing_in_map or extra_in_map:
        ...
        if extra_in_map:
            print(
                f"  NOTE: param_map contains extra keys not present in the UI (ignoring domain): {extra_in_map}"
            )
        print(
            "  Tip: keep UI labels in main.py and keys in param_map identical so flags map correctly."
        )
except Exception:
